...
class RecipeViewController: UIViewController {
    ...
    public static func makeFromStoryboard() -> RecipeViewController {
        let bundle = Bundle(for: RecipeViewController.self)
    ...
    override func viewWillLayoutSubviews() {
        ...
        // Adjust copy code button bottom constraint.
        // Note: codeButtonBottomConstraint is preconfigured to show the Copy Code button.
        var codeButtonBottomOffset = CGFloat(0)
        if let _ = equipmentSet {
            // Equipment set.
            copyCodeButtonHeightConstraint.constant = 0
        } else {
            // Item.
            if !canCopyCode {
                codeButtonBottomOffset -= copyCodeButton.frame.height
            }
        }
        if isEquippable {
            codeButtonBottomOffset += equipButton.frame.height
        }
        copyCodeButtonBottomConstraint.constant = codeButtonBottomOffset
    }
    ...
    static func presentRecipe(for forgedItem: ForgedItem, equipmentSet: EquipmentSet? = nil, from viewController: UIViewController, sourceView: UIView, equippable: Bool = false, delegate: RecipeViewControllerDelegate? = nil, arrowDirections: UIPopoverArrowDirection? = nil, layoutFrame: CGRect? = nil) {
        ...
        if let arrowDirections = arrowDirections {
            ...
            if let layoutFrame = layoutFrame {
                let popoverLayoutFrame = UIEdgeInsetsInsetRect(layoutFrame, popoverPresentationController?.popoverLayoutMargins ?? UIEdgeInsets.zero)
        ...
        viewController.present(recipeViewController, animated: true) {
            UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, recipeViewController.axWrapperView)
        }
    }
