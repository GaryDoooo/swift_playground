...
class Machine: UIView {
    ...
    func load() {
        ...
        if let missingLayersKeyPaths = animationView.getMissingLayersIn(keyPaths: allExpectedLayerKeyPaths) {
            for keyPath in missingLayersKeyPaths {
                PBLog(message: "Missing layer: \(keyPath)")
            }
        }
    ...
    func reveal(reduceMotion: Bool) {
        playSound(SoundFX.machineAppearingSound)
    ...
    func vocalize(completion: (() -> Void)? = nil) {
        playSound(SoundFX.idlingVocalization, volume: 5)
    ...
    func startIdlingSound() {
        audioController.playBackgroundAudioLoop(SoundFX.idling, volume: 15)
    }
    ...
    func forge(speed: Speed = .normal, reduceMotion: Bool, completion: (() -> Void)? = nil) {
        ...
        playSound( speed.isFast ? SoundFX.fastForgingSound : SoundFX.forgingSound)
    ...
    func equip(bodyPart: BodyPart, item: Thing, silent: Bool = false) {
        ...
        if !silent {
            playSound(SoundFX.equippedItemSound)
        }
        
        // TODO - jimt - This is getting called when the live view loads.
    ...
    @objc func describeTheMachine() {
        ...
        UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, description)
    }
    
    @objc func describeTheEquipment() {
        if Robot.bodyParts.filter({ $0.isEquipped }).count < 1 {
            UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, NSLocalizedString("The machine currently has nothing equipped.", comment: "AX description"))
        }
        else {
            var description = ""
            for bodyPart in Robot.bodyParts {
