...
public class SLOTComposition {
    ...
    convenience init?(filePath: URL) {
        let animationName = filePath.path
        ...
        do {
            ...
            // [will|did]Set observers on properties are not called in an initialization content. Deferring allows this to be set immediately after leaving initialization, which fires the observers.
            defer {
                rootDirectory = filePath.deletingLastPathComponent()
            }
        }
        catch {
            fatalError("SLOTComposition failed to init: \(error)")
