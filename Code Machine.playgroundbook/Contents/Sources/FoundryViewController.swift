...
public class FoundryViewController: UIViewController, PlaygroundLiveViewSafeAreaContainer {
    ...
    public static func makeFromStoryboard() -> FoundryViewController {
        let bundle = Bundle(for: FoundryViewController.self)
    ...
    override public func viewDidLoad() {
        ...
        leftBarButton.setImage(UIImage(named: "EquipMenuIcon"), for: .normal)
        updateAudioButton()
    ...
    private func updateAudioButton() {
        rightBarButton.setTitle(nil, for: .normal)
        let iconImage = audioController.isAudioEnabled ? UIImage(named: "AudioOn") : UIImage(named: "AudioOff")
        rightBarButton.accessibilityLabel = audioController.isAudioEnabled ?
        ...
        rightBarButton.setImage(iconImage, for: .normal)
    }
    ...
    private func configureLayout(for size: CGSize) {
        ...
        outerStackViewTopConstraint.constant = safeAreaTopHeight + CGFloat(buttonsAreHidden ? leftBarButtonTopSafeAreaConstraint.constant : 80)
        ...
        rightBarButton.isHidden = buttonsAreHidden
    ...
    private func startForgeCycle() {
        ...
        // Move AX focus to the machine: has the effect of announcing the recipe.
        if (UIAccessibilityIsVoiceOverRunning()) {
            let machineFocusAXElement = machine.focusAXElementForForging(recipe: stateMachine.currentRecipe)
            UIAccessibilityPostNotification(UIAccessibilityScreenChangedNotification, machineFocusAXElement)
        }
    ...
    private func animateForgeCycle(forgedItem: ForgedItem, speed: Speed = .normal, completion: (() -> Void)? = nil) {
        ...
        if speed.isNormal {
            playSound(SoundFX.enteringFunnel)
        } else if speed.isFast {
            playSound(Sound.fastEntering)
        }
    ...
    private func finishForgeCycle(forgedItem: ForgedItem) {
        ...
        let newlyAddedThingView = conveyorBeltViewController?.addItem(forgedItem: forgedItem)
        
        // Update secondary items to show forged items.
        updateSecondaryItemStatus()
        
        // Move AX focus to the newly forged item on the conveyor.
        if (UIAccessibilityIsVoiceOverRunning()) {
            if let newlyAddedThingView = newlyAddedThingView {
                UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, newlyAddedThingView)
            }
        }
        ...
        if forgedItem.item.isFinalProduct, forgedItem.isForgedFirstTime
        {
            ...
            if isEquipAlertsEnabled {
                ...
                equipConfirmationViewController.completion = { equipConfirmed in
                    ...
                    UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification, self.machineContainerView)
                }
                ...
                present(equipConfirmationViewController, animated: true, completion: {
                    if (UIAccessibilityIsVoiceOverRunning()) {
                        UIAccessibilityPostNotification(UIAccessibilityLayoutChangedNotification,
                                                        equipConfirmationViewController.view)
                    }
                })
    ...
    @IBAction func didTapRightBarButton(_ sender: Any) {
        audioController.isAudioEnabled = !audioController.isAudioEnabled
        updateAudioButton()
        
        if audioController.isAudioEnabled {
            machine.startIdlingSound()
        }
    }
...
extension FoundryViewController: PlaygroundLiveViewMessageHandler {
