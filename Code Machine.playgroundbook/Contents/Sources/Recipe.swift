...
public struct Recipe: Codable, Equatable {
    ...
    var accessibilityDescription: String? {
        ...
        let format : String
        if let _ = light {
            format = NSLocalizedString("%1$@ plus %2$@ with %3$@ light", comment: "AX description for recipe including a light")
        }
        else {
            format = NSLocalizedString("%1$@ plus %2$@", comment: "AX description for recipe")
        }
