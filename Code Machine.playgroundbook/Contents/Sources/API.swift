...
/**
 Inputs and outputs to the machine are known as items. Some items are final products that can be equipped on the machine.
...
public func setItemA(_ item: Item) {
    ...
    proxy.send(PlaygroundMessageToLiveView.setItemA(item: item).playgroundValue)
}
...
public func setItemB(_ item: Item) {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.setItemB(item: item).playgroundValue)
}

/**
Turns on a specific color of light on the machine. Lights must be set before `forgeItems()` is called.
 - parameters:
...
private func switchLight(light: Light, onOff: SwitchState) {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.switchLight(light: light, onOff: onOff).playgroundValue)
}
...
public func setSpeed(_ speed: Speed) {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.setSpeed(speed: speed).playgroundValue)
}
...
public func forgeItems() -> Item {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return .brick }
    proxy.send(PlaygroundMessageToLiveView.forgeItems.playgroundValue)
...
public func reset() {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.reset.playgroundValue)
}

/**
 Enable or disable equip alerts after forging new items.
 - parameters:
 - enabled: Set to `false` to turn off equip alerts. Default is `true`.
 - autoEquip: Set to `true` to automatically equip a forged item if (equip alerts) `enabled` is `false`. Optional. Default is `false`.
 - localizationKey: enableEquipAlerts(_:autoEquip:)
 */
public func enableEquipAlerts(_ enabled: Bool, autoEquip: Bool = false) {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.enableEquipAlerts(enabled: enabled, autoEquip: autoEquip).playgroundValue)
}
 ...
 */
public func playSoundInLiveView(_ sound: Sound) {
    guard let proxy = PlaygroundPage.current.liveView as? PlaygroundRemoteLiveViewProxy else { return }
    proxy.send(PlaygroundMessageToLiveView.playSound(sound: sound).playgroundValue)
}
