...
public class ToneSensor : AURenderCallbackDelegate {
    ...
    public init() {
        ...
        AVAudioSession.sharedInstance().requestRecordPermission () { allowed in
            if allowed {
                ...
                do {
                    try sessionInstance.setCategory(AVAudioSessionCategoryPlayAndRecord, with: [.allowBluetooth, .mixWithOthers]) // Support play and record to work well with ToneOutput.
    ...
    /// An update handler to collect the current sample from the tone sensor.
    ///
