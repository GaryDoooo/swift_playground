...
public class LiveViewGraphic: Equatable, Comparable {
    ...
    private func path(for image: Image) -> CGPath? {
        ...
        let points = stringPoints.map(CGPointFromString)
    ...
    private func size(for image: Image) -> CGSize? {
        ...
        let size = CGSizeFromString(stringSize)
    ...
    func updateTextImage() {
        ...
        guard let textImage = LiveViewGraphic.image(from: text, textColor: textColor, font: font) else { return }
