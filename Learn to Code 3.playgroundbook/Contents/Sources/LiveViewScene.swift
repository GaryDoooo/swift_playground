...
public class LiveViewScene: SKScene, ToolbarViewControllerDelegate, UIGestureRecognizerDelegate, GraphicAccessibilityElementDelegate, BackgroundAccessibilityElementDelegate {
    ...
    func handleMessage(message: Message) {
        ...
        case .moveAndRemove(let id, let position, let duration, let sound):
            moveAndRemove(id: id, to: position, duration: duration, sound: sound)
    ...
    func moveAndRemove(id: String, to position: CGPoint, duration: Double, sound: Sound?) {
        
        DispatchQueue.main.async {
            guard let graphic = self.graphicsInfo[id] else { return }
            graphic.move(to: position, duration: duration, sound: sound, completion: {
    ...
    func spinAndPop(id: String, after seconds: Double) {
        ...
        DispatchQueue.main.async {
            guard let graphic = self.graphicsInfo[id] else { return }
            graphic.spinAndPop(after: seconds, completion: {
                self.removeGraphic(id: id)
            })
        }
    }
    ...
    func swirlAway(id: String, after seconds: Double, rotations: Double) {
        ...
        DispatchQueue.main.async {
            guard let graphic = self.graphicsInfo[id] else { return }
            graphic.swirlAway(after: seconds, rotations: CGFloat(rotations), completion: {
                self.removeGraphic(id: id)
            })
            self.axElements.removeAll(keepingCapacity: true)
            UIAccessibilityPostNotification(UIAccessibilityScreenChangedNotification, self.accessibilityElements?.first)
    ...
    func setButton(name: String) {
        ...
        if name.count > maxNameLength { // arbitrary cutoff to keep the button name from getting too long
            userButtonName = name[..<name.index(name.startIndex, offsetBy: maxNameLength - 1)] + "…"
        }
        else {
            userButtonName = name
        }
    ...
    @objc func imageDetailsForFocusedQuadrant() {
