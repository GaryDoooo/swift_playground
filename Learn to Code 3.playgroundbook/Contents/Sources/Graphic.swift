...
public class Graphic: MessageControl {
    ...
    ///
    /// - Parameter scale: The scale to change to.
    /// - Parameter duration: The time over which to change scale.
    ///
    /// - localizationKey: Graphic.scaleTo(_:duration:)
    public func scaleTo(_ scale: Number, duration: Number = 0.0) {
        
        guard !suppressMessageSending else { return }
        let scaleAction = SKAction.scale(to: CGFloat(scale.double), duration: duration.double)
