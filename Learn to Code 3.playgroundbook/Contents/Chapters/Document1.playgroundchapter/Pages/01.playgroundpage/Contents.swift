 ...
 * callout(Blu is born):
 ...
 In code, the universe is an instance of `Scene` called `scene`. The scene has a [coordinate](glossary://coordinates) system, and any position on it can be specified by a `Point` value. You can use the `place()` method to place a graphic at a point on the scene.
