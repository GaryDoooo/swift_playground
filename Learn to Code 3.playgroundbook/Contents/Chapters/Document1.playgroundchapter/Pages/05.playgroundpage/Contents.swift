 ...
 **Goal:** Insert planet Rugbyball and remove planet Billiardball.
 ...
 You use the `insert()` method to add an item to an array at a specific [index](glossary://index). Any items after it move up one position, and the array’s `count` increases by 1.
 
 * callout(Inserting an item into an array):
    `images.insert(🌝, at: 2)`
 
 The `remove()` method removes an item at an index. Any items that were after it move down one position, and the `count` decreases by 1.
