...
public class LiveViewController: UIViewController, PlaygroundLiveViewSafeAreaContainer {
    ...
    @IBOutlet weak var pathLengthValue: UILabel!
    ...
    public override func viewDidLoad() {
        ...
        pathLengthLabel.text = NSLocalizedString("LENGTH OF PATH", comment: "The length of the path the algorithm has found from the start to the goal")
    ...
    private func configureStackViews(for size: CGSize) {
        ...
        contentContainerStackView.axis = viewIsPortrait ? .vertical : .horizontal
        labelStackView.axis = viewIsPortrait ? .horizontal : .vertical
        
        // Toggle visibility of views based on the state.
        if state == .completed {
            pathLengthValue.isHidden = false
            pathLengthLabel.isHidden = false
        }
        else {
            pathLengthLabel.isHidden = true
            pathLengthValue.isHidden = true
        }
    }
    ...
    public func play(_ steps: [MazeRunner.Step]) {
        ...
        let plotPath = BlockOperation {
            ...
            if let coordinates = pathDetails.coordinates {
                ...
                self.pathLengthValue.text = "\(coordinates.count - 1)" // -1 to not include the start coordinate.
            }
            else {
                self.pathLengthValue.text = NSLocalizedString("∞", comment: "Symbol showing the algorithm never found a path (infinite length)")
            }
